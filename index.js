 ...
 * @param {EventEmitter} eventSink - optional EventEmitter for the log events to be emitted upon. If omitted, a new EventEmitter will be created
 */
function Logger(namespace, eventSink) {
   ...
   * @param {String|bool} separator - string to separate chained namespaces. Pass boolean false if new beginning namespace is desired
  ...
  let methods = { log: log, withNamespace: withNamespace };

  for (let kind of KINDS) {
    methods[kind] = (message, err) => {
      return log(kind, message, err);
    };
  }
